function removeKthLinkedListNode(head, k) {
    ...
    const recurseRemove = (prevNode, currentNode) => {
        ...
        if (counterFromTail === k || (prevNode === head && counterFromTail + 1 === k)) {
            if (prevNode === head) {
                head = prevNode.next;
            } else {
...
function balancedBrackets(string) {
    ...
    const checkBrackets = stringArr.reduce((acc, character) => {
        ...
        if (character === "|") {
            console.log("bracketStack", bracketStack, "Character", character);
            if (bracketStack.length > 0 && bracketStack[bracketStack.length - 1] === character) {
                bracketStack.pop();
                return acc;
            } else {
            ...
            for (const thirdValue of arr) {
                if (secondValue !== thirdValue) {
                    if (currentValue + secondValue + thirdValue === target) {
                        const subArr = [currentValue, secondValue, thirdValue].sort((first, second) => first - second);
                        output.push(subArr);                       
                    }
                }
            } 
